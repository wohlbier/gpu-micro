...
void print_header() {
  PRINT("# %10s  %12s  %6s  %6s            out-of-place \
                      in-place          \n", "", "", "", "");
  PRINT("# %10s  %12s  %6s  %6s  %6s  %7s  %6s  %6s  %5s  %7s  %6s  %6s \
...
void CopyGetByteCount(size_t *sendcount, size_t *recvcount,
...
void CopyGetBuffSize(size_t *sendcount, size_t *recvcount, size_t count,
    ...
    CopyGetByteCount(sendcount, recvcount, &paramcount, &sendInplaceOffset,
